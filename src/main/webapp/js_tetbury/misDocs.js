 ...
 * @returns {undefined} */
function loadCarpetas(strCarpeta,element)
{
           //alert('entra'); 
     
     var url='';
    if(strCarpeta === 'carpetas'){
        url =  "ServletAjaxMyHD.servlet?accion=ListaCarpetasEmployee";
        $('#vistaCarpetas').addClass('active');
         $('#vistaSistema').removeClass('active');
    }
    else{
        url =  "ServletAjaxMyHD.servlet?accion=ListaCarpetasSistemaEmployee";
        $('#vistaSistema').addClass('active');
         $('#vistaCarpetas').removeClass('active');
    }
...
function loadCarpetasMover(){
           //alert('entra'); 
    

    //$('#listaCarpetas').empty();
    $.getJSON(
            "ServletAjaxMyHD.servlet?accion=ListaCarpetasEmployee",
            function(data)
            {
...
function CrearTablaTuplasDocs(myJson)
{
    ...
    for (j = 0; j <= (obj.length - 1); j++)
    {
        ...
        tabla.AddRowCellText(row, 3,           
           ...
           '<li><a onclick="cambiarIDDoc(\''  + obj[j].id +  '\')"  href="#modalClientes" class="btn tip" title="Compartir Documento" data-toggle="modal" docid="'+ obj[j].id+'"> <i class="icon-share-alt"> </i> </a></li>'+
                '</ul>');
...
function LeerDocs(xIDCarpeta)
{
    ...
    var dataToSend='accion=ListaDocumentosEmployee&xIDCarpeta='+xIDCarpeta+'&pagina='+pag +'&size='+tama;
...
function saveChangeCarpeta() {
    ...
    $.ajax({
                ...
                //alert('carpteta creada
                    $.jGrowl("El docuemento ha sido cambiado de carpeta");
            });
    idMoverCarpeta = 0;
}
...
function paginaAnterior(conn){
        ...
        //alert(window.selCarpeta);
         conn.PrevPage('accion=ListaDocumentosEmployee&xIDCarpeta='+window.selCarpeta,conn);
}

function paginaSiguiente(conn){
        //alert(window.selCarpeta);
    
          conn.NextPage('accion=ListaDocumentosEmployee&xIDCarpeta='+window.selCarpeta,conn); 
}
